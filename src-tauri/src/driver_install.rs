...
$instDir=Split-Path $inst.FullName;$log=Join-Path $base 'install.log';$sb=(Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\SecureBoot\State' -Name UEFISecureBootEnabled -ErrorAction SilentlyContinue).UEFISecureBootEnabled;$cmd='/c """'+$inst.FullName+'"" /install > """'+$log+'"" 2>&1"';$code=0;$cancelled=$false;try{$pr=Start-Process -FilePath 'cmd.exe' -ArgumentList $cmd -WorkingDirectory $instDir -Verb RunAs -Wait -PassThru;$code=$pr.ExitCode}catch{$cancelled=$true;$cancelMsg=$_.Exception.Message};$ilog='';try{$ilog=Get-Content $log -Raw -ErrorAction SilentlyContinue}catch{};if($ilog -and $ilog.Length -gt 800){$ilog=$ilog.Substring(0,800)};$drvPresent=(Test-Path 'C:\Windows\System32\drivers\interception.sys') -or (Test-Path 'C:\Windows\Sysnative\drivers\interception.sys') -or (Get-Service interception -ErrorAction SilentlyContinue) -or (Test-Path 'HKLM:\SYSTEM\CurrentControlSet\Services\interception');if($cancelled -and !$drvPresent){throw ('admin required:'+$cancelMsg)};if($drvPresent){'INTERCEPTION_INSTALL_OK:'+$dllFixed;return};if($code -ne 0){throw ('installer exit:'+$code+'. log:['+$ilog+']. SecureBoot='+$sb+'. driver not detected; try reboot, or admin cmd: install-interception.exe /install')};
